...
SELECT COUNT(*) AS Total_dir_mapping 
...
SELECT COUNT(*) AS Total_role_mapping 
FROM role_mapping;
...
SUM(CASE WHEN duration IS NULL THEN 1 ELSE 0 END) AS dur_null,
SUM(CASE WHEN country IS NULL THEN 1 ELSE 0 END) AS cntry_null,
...
SUM(CASE WHEN production_company IS NULL THEN 1 ELSE 0 END) AS prodcomp_null
FROM movie;
...
SELECT count(id) AS Total_movies_ind_usa
FROM movie
...
SELECT COUNT(movie_id) AS movie_med8_rat_Total
FROM ratings r
INNER JOIN movie m
ON r.movie_id =m.id
...
(
SELECT SUM(total_votes) AS german_tot_votes,
       ROW_NUMBER() OVER(ORDER BY SUM(total_votes)) AS id
FROM ratings r 
LEFT JOIN movie m
ON r.movie_id = m.id
...
italain_votes AS 
(
SELECT SUM(total_votes) AS italian_tot_votes,
       ROW_NUMBER() OVER(ORDER BY SUM(total_votes)) AS id
FROM ratings r 
LEFT JOIN movie m
ON r.movie_id = m.id
...
SELECT *,
       CASE WHEN german_tot_votes > italian_tot_votes THEN 'Yes'
       ELSE 'No' END AS german_votes_more_thanitalian 
       FROM german_votes g
       INNER JOIN italain_votes t
       USING(id);
...
SELECT     
m.production_company,Sum(r.total_votes) AS 'vote_count',
...
ON m.id = r.movie_id
WHERE  m.country 
LIKE '%India%' AND rm.category = 'actress' AND m.languages LIKE '%Hindi%'
GROUP  BY n.NAME
HAVING Count(m.id) >= 3
)
SELECT *, 
...
ON m.id= g.movie_id
GROUP BY genre window w1 AS (ORDER BY genre rows UNBOUNDED PRECEDING),w2 AS (ORDER BY genre rows 10 PRECEDING)
ORDER BY   genre;
...
Cast(Replace(Replace(IFNULL(worlwide_gross_income,0),',',''),'$','') AS DECIMAL(15,0)) AS amount,
DENSE_RANK() OVER(partition BY year ORDER BY Cast(Replace(Replace(IFNULL(worlwide_gross_income,0),',',''),'$','') AS DECIMAL(15,0)) DESC) AS movie_rank
FROM movie AS m
INNER JOIN genre AS g
...
SELECT n.NAME AS 'actress_name',
Sum(r.total_votes) AS 'total_votes', Count(m.id) AS 'movie_count',
...
INNER JOIN movie AS m
ON dm.movie_id=m.id ), movie_date_difference AS
(SELECT *,
Datediff(next_movie_date, date_published) AS diff
FROM next_movie_date_info ), avg_inter_days AS
(SELECT name_id,
Avg(diff) AS avg_inter_movie_days
FROM movie_date_difference
